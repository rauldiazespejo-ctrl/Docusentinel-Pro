...
// Registrar URLs blob para limpiarlas después
const _previewBlobUrls = [];

async function previewDocModal(docId, docName, docType) {
  // Limpiar blob URLs anteriores para evitar fugas de memoria
  while (_previewBlobUrls.length) {
    try { URL.revokeObjectURL(_previewBlobUrls.pop()); } catch(e) {}
  }

  // Abrir modal grande con spinner de carga
  showModalXL('modal-preview', `
    <div class="modal-header" style="background:var(--bg-card);border-bottom:1px solid var(--bg-border);padding:16px 20px;display:flex;align-items:center;justify-content:space-between;border-radius:12px 12px 0 0">
      <div class="modal-title" style="display:flex;align-items:center;gap:10px;font-size:15px;font-weight:700">
        <i class="fas fa-file-search" style="color:var(--cyan-400)"></i>
        <span style="max-width:400px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${docName}">${docName}</span>
      </div>
      <div style="display:flex;gap:8px;align-items:center">
        <button class="btn btn-primary btn-sm" onclick="downloadDoc('${docId}','${docName}')"><i class="fas fa-download"></i> Descargar</button>
        <button onclick="closeModal('modal-preview')" style="width:30px;height:30px;border:1px solid var(--bg-border);border-radius:6px;background:transparent;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text-secondary)"><i class="fas fa-times"></i></button>
      </div>
    </div>
    <div id="preview-body" style="min-height:300px;display:flex;align-items:center;justify-content:center;background:#0d1627;border-radius:0 0 12px 12px;overflow:hidden;position:relative">
      <div style="text-align:center">
        <div class="loading-dots" style="margin-bottom:12px"><div></div><div></div><div></div></div>
        <div style="color:var(--text-secondary);font-size:13px">Cargando vista previa…</div>
      </div>
    </div>`);
  ...
  if (!res.ok || !res.data || !res.data.success) {
    const errMsg = res.data?.error || 'No se pudo cargar la vista previa';
    body.innerHTML = `
      <div style="text-align:center;padding:32px">
        <i class="fas fa-exclamation-circle" style="font-size:3rem;color:#ef4444;margin-bottom:16px;display:block"></i>
        <div style="font-weight:600;margin-bottom:8px;color:var(--text-primary)">Error al cargar la vista previa</div>
        <div style="color:var(--text-secondary);font-size:13px;margin-bottom:20px">${errMsg}</div>
        <button class="btn btn-primary btn-sm" onclick="downloadDoc('${docId}','${docName}')"><i class="fas fa-download"></i> Descargar archivo</button>
      </div>`;
    return;
  }
  ...
  if (!d.previewAvailable) {
    body.innerHTML = `
      <div style="text-align:center;padding:40px">
        <div style="font-size:4rem;margin-bottom:16px">${fileIcon(docType)}</div>
        <div style="font-weight:700;font-size:16px;margin-bottom:8px;color:var(--text-primary)">${d.name || docName}</div>
        <div style="color:var(--text-secondary);font-size:13px;margin-bottom:24px">${d.message || 'Vista previa no disponible para este tipo de archivo'}</div>
        <button class="btn btn-primary" onclick="downloadDoc('${docId}','${docName}')"><i class="fas fa-download"></i> Descargar para ver</button>
      </div>`;
    return;
  }
  ...
  if (d.previewType === 'image') {
    body.style.cssText = 'background:#111;border-radius:0 0 12px 12px;overflow:hidden;display:flex;align-items:center;justify-content:center;min-height:300px;padding:0';
    const img = document.createElement('img');
    img.src = d.dataUrl;
    img.alt = d.name || docName;
    img.style.cssText = 'max-width:100%;max-height:70vh;object-fit:contain;display:block';
    img.onerror = () => {
      body.innerHTML = `<div style="text-align:center;padding:32px;color:#ef4444"><i class="fas fa-image" style="font-size:3rem;margin-bottom:12px;display:block"></i>No se pudo mostrar la imagen</div>`;
    };
    body.innerHTML = '';
  ...
  if (d.previewType === 'pdf') {
    body.style.cssText = 'background:#525659;border-radius:0 0 12px 12px;overflow:hidden;min-height:500px;padding:0;position:relative;display:block';
    try {
      // Convertir data URI → Blob URL para evitar bloqueos de CSP/navegador
      const base64Data = d.dataUrl.split(',')[1];
      if (!base64Data) throw new Error('dataUrl vacío');
      ...
      // Intentar con <object> primero (más compatible que iframe para PDFs)
      body.innerHTML = `
        <object data="${blobUrl}" type="application/pdf" style="width:100%;height:70vh;display:block;border:none">
          <embed src="${blobUrl}" type="application/pdf" style="width:100%;height:70vh;display:block;border:none">
            <div style="text-align:center;padding:40px;color:var(--text-primary)">
              <i class="fas fa-file-pdf" style="font-size:3rem;color:#ef4444;margin-bottom:16px;display:block"></i>
              <p style="margin-bottom:20px">Tu navegador no puede mostrar PDFs en línea.</p>
        ...
        <div style="position:absolute;bottom:12px;right:12px;z-index:10;display:flex;gap:8px">
          <button class="btn btn-secondary btn-sm" style="background:rgba(0,0,0,0.6);border-color:rgba(255,255,255,0.3);color:#fff" 
            onclick="window.open('${blobUrl}','_blank')">
            <i class="fas fa-external-link-alt"></i> Nueva pestaña
          </button>
          <button class="btn btn-secondary btn-sm" style="background:rgba(0,0,0,0.6);border-color:rgba(255,255,255,0.3);color:#fff" 
            onclick="downloadDoc('${docId}','${docName}')">
      ...
      showPdfFallback(body, null, docId, docName);
    }
    return;
  }
  ...
  body.innerHTML = `
    <div style="text-align:center;padding:40px">
      <div style="font-size:4rem;margin-bottom:16px">${fileIcon(docType)}</div>
      <div style="font-weight:600;margin-bottom:8px;color:var(--text-primary)">${docName}</div>
      <div style="color:var(--text-secondary);font-size:13px;margin-bottom:24px">Vista previa no disponible para este formato (${docType})</div>
      <button class="btn btn-primary" onclick="downloadDoc('${docId}','${docName}')"><i class="fas fa-download"></i> Descargar archivo</button>
    </div>`;
}

function showPdfFallback(body, blobUrl, docId, docName) {
  body.innerHTML = `
    <div style="text-align:center;padding:40px">
      <i class="fas fa-file-pdf" style="font-size:4rem;color:#ef4444;margin-bottom:16px;display:block"></i>
      <div style="font-weight:700;font-size:16px;margin-bottom:8px;color:var(--text-primary)">${docName}</div>
      <div style="color:var(--text-secondary);font-size:13px;margin-bottom:24px">Tu navegador no puede mostrar PDFs en línea directamente.</div>
      <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
        ${blobUrl ? `<button class="btn btn-primary" onclick="window.open('${blobUrl}','_blank')"><i class="fas fa-external-link-alt"></i> Abrir en nueva pestaña</button>` : ''}
        <button class="btn btn-secondary" onclick="downloadDoc('${docId}','${docName}')"><i class="fas fa-download"></i> Descargar PDF</button>
      </div>
    </div>`;
}
  ...
  <div class="login-page">
    ...
    <div class="login-card">
      ...
      <div id="form-login">
        ...
        <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:12px;margin-top:8px" id="login-btn">
      ...
      <div id="form-register" class="hidden">
        ...
        <div class="form-group">
          ...
          <div class="input-group">
            ...
            <input class="form-input" type="text" id="r-name" placeholder="Tu nombre completo" autocomplete="name">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Correo electrónico</label>
          <div class="input-group">
            <i class="fas fa-envelope input-icon"></i>
            <input class="form-input" type="email" id="r-email" placeholder="tu@empresa.com" autocomplete="email">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Contraseña</label>
          <div class="input-group">
            <i class="fas fa-lock input-icon"></i>
            <input class="form-input" type="password" id="r-pass" placeholder="Mínimo 8 caracteres" autocomplete="new-password">
            <i class="fas fa-eye input-action" onclick="togglePass('r-pass',this)"></i>
          </div>
        </div>
        <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:12px;margin-top:8px" id="register-btn">
...
function showModalXL(id, content) {
  // Modal extra grande para previsualizaciones (sin padding en body propio del modal)
  let overlay = document.getElementById(id);
  if (!overlay) {
    overlay = document.createElement('div');
    overlay.id = id;
    overlay.className = 'modal-overlay';
    overlay.innerHTML = `<div class="modal" style="max-width:900px;width:96vw;max-height:90vh;overflow:hidden;padding:0;border-radius:12px">${content}</div>`;
    overlay.addEventListener('click', e => { if (e.target===overlay) closeModal(id); });
    document.body.appendChild(overlay);
  } else {
    overlay.querySelector('.modal').innerHTML = content;
...
window.showPdfFallback = showPdfFallback;
