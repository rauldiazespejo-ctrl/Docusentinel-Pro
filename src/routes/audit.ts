    ...
    try {
      ...
      const normalizedLogs = logs.map((l: any) => {
        ...
        return {
          ...
          actor_role: (() => { const roleMap: Record<number,string> = {1:'Administrador',2:'Editor',3:'Viewer',4:'Auditor'}; const r = l.actorRole; return typeof r === 'number' ? (roleMap[r] || `Rol ${r}`) : String(r || ''); })(),
